...
AddEventHandler('::{korioz#0110}::BanSql:ICheatClient', function(reason)
	...
	if _source then
		...
		if name then
			...
			AddBan(_source, licenseid, playerip, name, 'Anti-Cheat NeverLife', 0, reason, 1)
			DropPlayer(_source, ('Vous êtes banni de NeverLife\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat NeverLife'):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat NeverLife have received invalid id.')
	end
end)
...
AddEventHandler('::{korioz#0110}::BanSql:ICheatServer', function(target, reason)
	...
	if target then
		...
		if name then
			...
			AddBan(target, licenseid, playerip, name, 'Anti-Cheat NeverLife', 0, reason, 1)
			DropPlayer(target, ('Vous êtes banni de NeverLife\nRaison : %s\nTemps Restant : Permanent\nAuteur : Anti-Cheat NeverLife'):format(reason))
		end
	else
		print('BanSql Error : Anti-Cheat NeverLife have received invalid id.')
