...
AddEventHandler('::{korioz#0110}::esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showAdvancedNotification("NeverLife", "~y~Inventaire", _U('gave_item', itemCount, ESX.Items[itemName].label, targetXPlayer.name), 'CHAR_NEVERLIFE', 7)
				targetXPlayer.showAdvancedNotification("NeverLife", "~y~Inventaire", _U('received_item', itemCount, ESX.Items[itemName].label, sourceXPlayer.name), 'CHAR_NEVERLIFE', 7)

				TriggerEvent("::{korioz#0110}::esx:giveitemalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
			else
				sourceXPlayer.showAdvancedNotification("NeverLife", "~y~Inventaire", _U('ex_inv_lim', targetXPlayer.name), 'CHAR_NEVERLIFE', 7)
			end
		else
			sourceXPlayer.showAdvancedNotification("NeverLife", "~y~Inventaire", _U('imp_invalid_quantity'), 'CHAR_NEVERLIFE', 7)
		end
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showAdvancedNotification("NeverLife", "~y~Portefeuille", _U('gave_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, targetXPlayer.name), 'CHAR_NEVERLIFE', 9)
			targetXPlayer.showAdvancedNotification("NeverLife", "~y~Portefeuille", _U('received_account_money', ESX.Math.GroupDigits(itemCount), accountLabel, sourceXPlayer.name), 'CHAR_NEVERLIFE', 9)

			TriggerEvent("::{korioz#0110}::esx:giveaccountalert", sourceXPlayer.name, targetXPlayer.name, itemName, itemCount)
		else
			sourceXPlayer.showAdvancedNotification("NeverLife", "~y~Portefeuille", _U('imp_invalid_amount'), 'CHAR_NEVERLIFE', 9)
		end
	elseif type == 'item_weapon' then
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if itemCount > 0 then
					sourceXPlayer.showAdvancedNotification("NeverLife", "~y~Armes", _U('gave_weapon_withammo', weaponLabel, itemCount, targetXPlayer.name), 'CHAR_NEVERLIFE', 7)
					targetXPlayer.showAdvancedNotification("NeverLife", "~y~Armes", _U('received_weapon_withammo', weaponLabel, itemCount, sourceXPlayer.name), 'CHAR_NEVERLIFE', 7)
				else
					sourceXPlayer.showAdvancedNotification("NeverLife", "~y~Armes", _U('gave_weapon', weaponLabel, targetXPlayer.name), 'CHAR_NEVERLIFE', 7)
					targetXPlayer.showAdvancedNotification("NeverLife", "~y~Armes", _U('received_weapon', weaponLabel, sourceXPlayer.name), 'CHAR_NEVERLIFE', 7)

					TriggerEvent("::{korioz#0110}::esx:giveweaponalert", sourceXPlayer.name, targetXPlayer.name, itemName)
				end
			else
				sourceXPlayer.showAdvancedNotification("NeverLife", "~y~Armes", _U('gave_weapon_hasalready', targetXPlayer.name, weaponLabel), 'CHAR_NEVERLIFE', 7)
				targetXPlayer.showAdvancedNotification("NeverLife", "~y~Armes", _U('received_weapon_hasalready', sourceXPlayer.name, weaponLabel), 'CHAR_NEVERLIFE', 7)
			end
		end
...
AddEventHandler('::{korioz#0110}::esx:dropInventoryItem', function(type, itemName, itemCount)
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("NeverLife", "~y~Inventaire", _U('imp_invalid_quantity'), 'CHAR_NEVERLIFE', 7)
		else
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				xPlayer.showAdvancedNotification("NeverLife", "~y~Inventaire", _U('threw_standard', itemCount, ESX.Items[itemName].label), 'CHAR_NEVERLIFE', 7)
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			xPlayer.showAdvancedNotification("NeverLife", "~y~Portefeuille", _U('imp_invalid_amount'), 'CHAR_NEVERLIFE', 9)
		else
			...
			if (itemCount > account.money or account.money < 1) then
				...
				xPlayer.showAdvancedNotification("NeverLife", "~y~Portefeuille", _U('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(accountLabel)), 'CHAR_NEVERLIFE', 9)
			end
		end
	elseif type == 'item_weapon' then
		itemName = string.upper(itemName)
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if weapon.ammo > 0 then
				xPlayer.showAdvancedNotification("NeverLife", "~y~Armes", _U('threw_weapon_ammo', weapon.label, weapon.ammo), 'CHAR_NEVERLIFE', 7)
			else
				xPlayer.showAdvancedNotification("NeverLife", "~y~Armes", _U('threw_weapon', weapon.label), 'CHAR_NEVERLIFE', 7)
			end
		end
	end
end)
...
AddEventHandler('::{korioz#0110}::esx:useItem', function(itemName)
	...
	if xItem then
		if xItem.count > 0 then
			...
			xPlayer.showAdvancedNotification("NeverLife", "~y~Inventaire", _U('act_imp'), 'CHAR_NEVERLIFE', 7)
		end
	else
