	...
	MySQL.Async.fetchAll('SELECT * FROM shops LEFT JOIN items ON items.name = shops.item', {}, function(data)
		for i = 1, #data, 1 do
			if data[i].name then
				...
				print(('::{korioz#0110}::esx_shops: invalid item "%s" found!'):format(data[i].item))
			end
		end
	end)
end)
...
AddEventHandler('::{korioz#0110}::esx_shops:buyItem', function(itemName, amount, zone)
	...
	if amount < 0 then
		print('::{korioz#0110}::esx_shops: ' .. xPlayer.identifier .. ' attempted to exploit the shop!')
		return
	end
...
RegisterServerEvent('::{korioz#0110}::esx_shops:policeNotif')
AddEventHandler('::{korioz#0110}::esx_shops:policeNotif', function(store, coords)
	local _source = source
	local xPlayers = ESX.GetPlayers()
	...
	for i = 1, #xPlayers do
		...
		if xPlayer.job.name == 'police' then
			TriggerClientEvent('::{korioz#0110}::esx_shops:msgPolice', xPlayer.source, store, coords)
		end
	end
end)
