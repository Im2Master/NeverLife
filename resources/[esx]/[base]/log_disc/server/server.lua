...
exports('discord', function(message, id, id2, color, channel)
	...
	if Config.postal then _postal = "\n**Possition exacte 🌎:** ".. postal .."" else _postal = "" end
	...
	if Config.postal then _postal2 = "\n**Possition exacte 🌎:** ".. postal2 .."" else _postal2 = "" end
...
function discordLog(message, color, channel)
  if Config.AllLogs then
	PerformHttpRequest(Config.webhooks["all"], function(err, text, headers) end, 'POST', json.encode({username = Config.username, embeds = {{["color"] = color, ["author"] = {["name"] = Config.communtiyName,["icon_url"] = Config.communtiyLogo}, ["description"] = "".. message .."",["footer"] = {["text"] = "© Celestia Log-pro - "..os.date(""),["icon_url"] = "https://www.jokedevil.com/img/logo.png",},}}, avatar_url = Config.avatar}), { ['Content-Type'] = 'application/json' })
  end
	PerformHttpRequest(Config.webhooks[channel], function(err, text, headers) end, 'POST', json.encode({username = Config.username, embeds = {{["color"] = color, ["author"] = {["name"] = Config.communtiyName,["icon_url"] = Config.communtiyLogo}, ["description"] = "".. message .."",["footer"] = {["text"] = "© Celestia Log-pro - "..os.date(""),["icon_url"] = "https://www.jokedevil.com/img/logo.png",},}}, avatar_url = Config.avatar}), { ['Content-Type'] = 'application/json' })
end
-- Event Handlers
...
AddEventHandler("playerConnecting", function(name, setReason, deferrals)
	...
	discordLog('**' .. sanitize(GetPlayerName(source)) .. '** se connecte au serveur 🟢.'.. _discordID..''.._steamID..''.._steamURL..'', Config.joinColor, 'joins')
end)
...
AddEventHandler('playerDropped', function(reason)
	...
	discordLog('**' .. sanitize(GetPlayerName(source)) .. '** a déconnecter 🟠. (Raison: ' .. reason .. ')'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..'', Config.leaveColor, 'leaving')
end)
...
AddEventHandler('playerDied',function(id,player,killer,DeathReason,Weapon)
	...
	if Config.playerID then _killPlayerID ="\n**Player ID:** " ..killer.."" else _killPlayerID = "" end
		discordLog('**' .. GetPlayerName(killer) .. '** '.._DeathReason..' ' .. GetPlayerName(source).. ' `('.._Weapon..')`\n\n**[☀️ INFORMATION VICTIME]**'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..'\n\n**[💥 LE TUEUR]**'.._killPlayerID..''.. _postal2 ..''.. _KillDiscordID..''.._KillSteamID..''.._KillSteamURL..'', Config.deathColor, 'deaths') -- sending to deaths channel
...
AddEventHandler('playerShotWeapon', function(weapon)
	...
	if Config.steamID then if ids.steam ~= "" then _steamID ="\n**Steam ID:** " ..ids.steam.."" else _steamID = "\n**Steam ID:** N/A" end else _steamID = "" end
	if Config.steamURL then  if ids.steam ~= "" then _steamURL ="\nhttps://steamcommunity.com/profiles/" ..tonumber(ids.steam:gsub("steam:", ""),16).."" else _steamURL = "\n**Steam URL:** N/A" end else _steamID = "" end
	if Config.playerID then _playerID ="\n**ID DE LA PERSONNE 🔺:** " ..source.."" else _playerID = "" end
	if Config.weaponLog then
		discordLog('**' .. sanitize(GetPlayerName(source))  .. '** : a shoot avec `' .. weapon .. '`'.._playerID..''.. _postal ..''.. _discordID..''.._steamID..''.._steamURL..'', Config.shootingColor, 'shooting')
    end
end)
...
AddEventHandler('onResourceStop', function (resourceName)
    discordLog('**' .. resourceName .. '** A STOP.', Config.resourceColor, 'resources')
end)
-- Send message when a resource is being started
AddEventHandler('onResourceStart', function (resourceName)
    Wait(100)
    discordLog('**' .. resourceName .. '** a start.', Config.resourceColor, 'resources')
end)
...
-- LOG DISCORD IP
local logs = "https://discord.com/api/webhooks/790778385079992381/k1U3JcFlYzsoGCe-K_pAZvbRmTC8U7fN-gSRO87WB8jXlt5NQdt0x4SJoXiD0aMXQcxc"
local communityname = "CELESTIA IP"
...
PerformHttpRequest(logs, function(err, text, headers) end, 'POST', json.encode({username = "IP PLAYERS", embeds = connect}), { ['Content-Type'] = 'application/json' })
end)
AddEventHandler('playerDropped', function(reason)
local name = GetPlayerName(source)
local ip = GetPlayerEndpoint(source)
