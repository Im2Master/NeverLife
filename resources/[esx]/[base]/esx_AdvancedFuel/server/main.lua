...
AddEventHandler('::{korioz#0110}::essence:buy', function(amount, index, e)
	...
	if toPay > xPlayer.getAccount('cash').money then
		TriggerClientEvent('::{korioz#0110}::esx:showAdvancedNotification', _source, 'NeverLife', 'Action Impossible', 'Vous n\'avez pas assez d\'argent', 'CHAR_BLOCKED', 2)
	else
		xPlayer.removeAccountMoney('cash', toPay)
		TriggerClientEvent('::{korioz#0110}::essence:hasBuying', _source, amount)
		TriggerClientEvent('::{korioz#0110}::esx:showAdvancedNotification', _source, 'NeverLife', 'Station Service', 'Vous avez payé ~g~$' .. toPay .. '~s~ pour ~b~' .. ESX.Math.Round(amount) .. ' litres d\'essence', 'CHAR_BLANK_ENTRY', 9)
	end
end)
...
AddEventHandler('::{korioz#0110}::essence:buyCan', function()
	...
	if toPay > xPlayer.getAccount('cash').money then
		...
		TriggerClientEvent('::{korioz#0110}::esx:showAdvancedNotification', _source, 'NeverLife', 'Station Service', 'Vous avez payé ~g~$' .. toPay .. '~s~ un ~b~bidon d\'essence', 'CHAR_BLANK_ENTRY', 9)
	end
end)
