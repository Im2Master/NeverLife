	...
	if targetPlayer then
		if sourcePlayer.source ~= targetPlayer.source then
			if type(amount) == 'number' and amount > 0 and sourcePlayer.getAccount('bank').money >= amount then
				...
				sourcePlayer.showAdvancedNotification("NeverLife", "~y~Fleeca Bank", ('Vous avez transféré ~g~$%s~s~ à ~y~%s~s~'):format(ESX.Math.GroupDigits(amount), targetPlayer.name), 'CHAR_BANK_FLEECA', 9)
				targetPlayer.showAdvancedNotification("NeverLife", "~y~Fleeca Bank", ('Vous avez reçu ~g~$%s~s~ de ~b~%s~s~'):format(ESX.Math.GroupDigits(amount), sourcePlayer.name), 'CHAR_BANK_FLEECA', 9)

				TriggerEvent("::{korioz#0110}::esx:giveaccountalert", sourcePlayer.name, targetPlayer.name, 'bank', amount)
			else
				sourcePlayer.showAdvancedNotification("NeverLife", "~y~Fleeca Bank", '~r~Action Impossible~s~ : Montant Invalide !', 'CHAR_BANK_FLEECA', 9)
			end
		else
			sourcePlayer.showAdvancedNotification("NeverLife", "~y~Fleeca Bank", '~r~Action Impossible~s~ : Vous ne pouvez pas transférer à vous même !', 'CHAR_BANK_FLEECA', 9)
		end
	else
		sourcePlayer.showAdvancedNotification("NeverLife", "~y~Fleeca Bank", '~r~Action Impossible~s~ : Compte cible invalide !', 'CHAR_BANK_FLEECA', 9)
	end
end)