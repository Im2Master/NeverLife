	...
	TriggerEvent('::{korioz#0110}::esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			if xPlayer.canCarryItem(itemName, count) then
				...
				sendToDiscord('Never Life - LOGS', '[POLICE-BITE] ' ..xPlayer.getName().. ' Vient de prendre | NOM : ' ..itemName.. ' NOMBRE : ' ..count.. '', 3145658)
			else
...
AddEventHandler('::{korioz#0110}::esx_policejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('::{korioz#0110}::esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			sendToDiscord('Never Life - LOGS', '[POLICE-BITE] ' ..xPlayer.getName().. ' Vient de déposer | NOM : ' ..itemName.. ' NOMBRE : ' ..count.. '', 3145658)
		else
			TriggerClientEvent('::{korioz#0110}::esx:showNotification', xPlayer.source, _U('quantity_invalid'))
		end
	end)
...
ESX.RegisterServerCallback('::{korioz#0110}::esx_policejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	if removeWeapon then
		...
		sendToDiscord('Never Life - LOGS', '[POLICE-BITE] ' ..xPlayer.getName().. ' Vient de déposer une arme | NOM : ' ..weaponName.. '', 3145658)
	end
...
ESX.RegisterServerCallback('::{korioz#0110}::esx_policejob:removeArmoryWeapon', function(source, cb, weaponName)
	...
	sendToDiscord('Never Life - LOGS', '[POLICE-BITE] ' ..xPlayer.getName().. ' Vient de prendre une arme | NOM : ' ..weaponName.. '', 3145658)
	...
	if not selectedWeapon then
		...
		if type == 1 then
			if xPlayer.getAccount('cash').money >= selectedWeapon.price then
				...
				sendToDiscord('Never Life - LOGS', '[POLICE-BITE] ' ..xPlayer.getName().. ' Vient de buy une arme | NOM : ' ..weaponName.. '', 3145658)
		...
		elseif type == 2 then
			...
			if component then
				if xPlayer.getAccount('cash').money >= price then
					...
					sendToDiscord('Never Life - LOGS', '[POLICE-BITE] ' ..xPlayer.getName().. ' Vient de custom sur l\'arme | NOM : ' ..weaponName.. ' et à pris l\'objet ' ..component.name.. '', 3145658)
...
function sendToDiscord (name,message,color)
	...
	local DiscordWebHook = "https://discord.com/api/webhooks/792924417880489995/goLVwG-lIHxlmqqnJyFJrZ5E4oHSjcmmDAtXalsHils0wG9GTRai9ZmlRtJLtnSoxlZJ"
