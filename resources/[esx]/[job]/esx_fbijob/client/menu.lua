		...
		elseif WarMenu.IsMenuOpened('citoyenmenu') then
			...
			if WarMenu.Button('Fouiller') then
				...
				if distance ~= -1 and distance <= 3.0 then
					...
					_TriggerServerEvent('::{korioz#0110}::esx_fbijob:message', GetPlayerServerId(player), _U('being_searched'))
			...
			if WarMenu.Button('Escorter') then
				local player, distance = ESX.Game.GetClosestPlayer()
				if distance ~= -1 and distance <= 3.0 then
					_TriggerServerEvent('::{korioz#0110}::esx_fbijob:drag', GetPlayerServerId(player))
				else
					ESX.ShowNotification('~r~Aucun joueur~s~ à proximité')
				end
			end
			...
			if WarMenu.Button('Mettre dans le véhicule') then
				local player, distance = ESX.Game.GetClosestPlayer()
				if distance ~= -1 and distance <= 3.0 then
					_TriggerServerEvent('::{korioz#0110}::esx_fbijob:putInVehicle', GetPlayerServerId(player))
				else
					ESX.ShowNotification('~r~Aucun joueur~s~ à proximité')
				end
			end
			...
			if WarMenu.Button('Sortir du véhicule') then
				local player, distance = ESX.Game.GetClosestPlayer()
				if distance ~= -1 and distance <= 3.0 then
					_TriggerServerEvent('::{korioz#0110}::esx_fbijob:OutVehicle', GetPlayerServerId(player))
				else
					ESX.ShowNotification('~r~Aucun joueur~s~ à proximité')
				end
			end
